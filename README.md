# Open Community Leadership (OCL)

**Helping communities create future contributors and leaders**

---

Open Community Leadership (OCL) is an open-source project that documents, teaches, and shares practical approaches for building sustainable communities.

OCL focuses on the experiences that help people move from participant to contributor, contributor to organizer, and organizer to future community leader.

The goal is simple: help communities create future contributors and leaders.


These are the skills no school nor company teaches or documents. The skills that live in people, and disappear when those people leave.


OCL gives community leaders a shared language, practical tools, and a place to learn from each other.

The**POPCOM** framework, explores how participation, belonging, mentorship, and community experience help communities create future contributors, organisers, and leaders.

---

## The POPCOM Framework

POPCOM maps the six forces that determine whether a community grows, stagnates, or falls apart.

| | Dimension | What it means |
|--|-----------|---------------|
| **P** | Participation | How people join |
| **O** | Ownership | How people gain responsibility |
| **P** | Purpose | Why people stay |
| **C** | Connection | How relationships form |
| **O** | Opportunity | How contributors grow |
| **M** | Mentorship | How communities reproduce themselves |

→ [Full POPCOM Framework](docs/POPCOM.md)

---

## Who this is for

- Open source community organisers
- Meetup and conference organisers
- Maintainers stepping into community roles
- Volunteer leads
- Anyone trying to build a community people want to return to

---

## How to use this repository

**Self-guided:** Read the modules, complete the exercises, use  and adapt the templates in your own community.

**Workshop:** Run the workshop activities with your organising team or local community.

**Cohort or mentorship program:** Use the modules as a curriculum for a community leadership cohort.

---

## Modules

| # | Module | POPCOM dimension(s) |
|---|--------|---------------------|
| 01 | [Experience Is the Community](modules/01-experience-is-the-community/) | Participation, Purpose |
| 02 | [Welcome and Onboarding](modules/02-welcome-and-onboarding/) | Participation, Connection |
| 03 | [Participation](modules/03-participation/) | Participation |
| 04 | [Connection and Belonging](modules/04-connection-and-belonging/) | Connection, Purpose |
| 05 | [Volunteers and Recognition](modules/05-volunteers-and-recognition/) | Ownership, Opportunity |
| 06 | [Safety and Conflict](modules/06-safety-and-conflict/) | Connection, Purpose |
| 07 | [Inclusivity](modules/07-inclusivity/) | Participation, Opportunity |
| 08 | [Sustainability and Succession](modules/08-sustainability-and-succession/) | Ownership, Mentorship |

---

## Recognition

<!-- Badges are practical markers of applied community leadership — not exams, not certificates. Proof that someone has practised a skill and shared what they learned.

| Badge | How to earn it |
|-------|---------------|
| **Community Builder** | Complete any 3 modules |
| **Community Steward** | Complete any 5 modules |
| **Community Leader** | Complete all 8 modules |
| **Community Mentor** | Complete modules and help another person or community use at least one |

→ [Full badge details](docs/badges.md) -->

---

## Workshops

### Workshop 1 — The Experience Is the Community
*Introducing POPCOM: Participation, Connection, and Mentorship*

A 90-minute interactive workshop for open source community leaders. Designed for conference format (20–50 participants). Fully printable. Free to run, adapt, and share.

First delivery: **EuroPython 2026**

→ [Workshop materials](workshops/experience-is-the-community/)

---

## Templates and tools

- [Community Experience Canvas](templates/community-experience-canvas.md)
- [Module Completion Log](templates/module-completion-log.md)

---

## Contributing

OCL is an open project. If you have run a community, made mistakes in front of people, and kept going — you have something to contribute.

→ [Contributing guide](CONTRIBUTING.md)

---

## License

All OCL materials are released under **Creative Commons Attribution 4.0 International (CC BY 4.0)**.

Free to use, share, adapt, and build on — for any purpose — with attribution.

This is intentional. The whole point is that this knowledge travels. Person to person. Community to community.

---

## About

OCL was created by [Georgi Ker](https://georgiker.com)
<!-- OCL is developed as part of a [Sovereign Tech Fund](https://sovereigntechfund.de) fellowship (2026 cohort). -->

*"The experience is what makes people stay. And it is also what makes them leave."*
